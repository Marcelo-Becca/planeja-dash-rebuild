...
export function TeamCard({
  ...
  return <Card className={`group hover:shadow-lg transition-all duration-300 border-l-4 ${getTeamColorClass(team.color)} bg-card/50 backdrop-blur-sm`}>
      ...
      <CardContent className="pt-0">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            ...
            {team.projects.length > 0 && <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex -space-x-2">
              {team.members.slice(0, 3).map(member => <Avatar key={member.id} className="h-8 w-8 border-2 border-background">
                  <AvatarFallback className="text-xs bg-primary/10 text-primary">
                    {member.user.avatar}
                  </AvatarFallback>
                </Avatar>)}
              {team.members.length > 3 && <div className="h-8 w-8 rounded-full border-2 border-background bg-muted flex items-center justify-center">
          ...
          {/* Main Project */}
          {team.projects[0] && <div className="pt-2 border-t border-border/50">
              <p className="text-xs text-muted-foreground mb-1">Projeto principal:</p>
              <Link to={`/projects/${team.projects[0].id}`} className="text-sm font-medium hover:text-primary transition-colors hover:underline">
